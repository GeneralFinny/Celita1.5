...
def play_audio(text, lang='tl'):
    ...
    audio_file_path = f'translated_audio_{timestamp}.mp3'  # Include the timestamp in the filename
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # Check if the input contains only a single word (no spaces)
        if ' ' not in user_input:
            # Use Google Translate for single-word input
            translated_text = translate_to_cebuano(user_input)
            play_audio(translated_text, lang='tl')  # Use 'tl' for Tagalog
            print("Translated:", translated_text)
        else:
            # Translate the input and play the audio with the 'tl' (Tagalog) language code
